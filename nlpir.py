...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        print "sys.argv[2]: stopwords file!"
        print "sys.argv[3]: output file path!"
        exit()
    
    #read stopwords from file
    #notice, it's import to remove '\n' in each word
    stopwords = []
    with open(sys.argv[2], "r") as fin:
        content = fin.readlines()
        for each in content:
            stopwords.append(each.strip().split('\n')[0])
        #print stopwords
    
    #read every file from input file path & write it to output file path
    files = [f for f in listdir(sys.argv[1]) if isfile(join(sys.argv[1], f))]
    for file in files:
        readPath = sys.argv[1] + file
        writePath = sys.argv[3] + file
        f = open(readPath, "r")
        #notice, here is "a+"
        result = open(writePath, "a+")
        content = f.readlines()
        for line in content:             
            for word in Seg(line):
    ...
    for t in Seg(p):
        #t[0] word, t[1] posTag, t[2] Chinese-posTag
